...
int iterate_folders(char *relative_CODE_path_in_each_folder,
  ...
  while ((dir = readdir(d)) != NULL && ERROR_CODE == EXIT_SUCCESS) {
    ...
    if (relative_CODE_path_in_each_folder[0] != '/' &&
        relative_CODE_path_in_each_folder[0] != '\\')
      strcat(code_path, "/");
...
int right_check_unwated(FILE *file, char *to_skip) {
  ...
  // While there are characters to_skip between the current position (new line)
...
int left_check_unwated(FILE *file, char *to_skip) {
  // printf("l\n");
  char current_character;
  // While there are characters to_skip between the text and the current
  // position (new line).
